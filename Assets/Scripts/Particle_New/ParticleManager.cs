...
public class ParticleManager : MonoBehaviour
{
    ...
    [Header("== PARTICLE CONFIGURATIONS ==")]
        ...
        private int maxNumParticles = 100000;
        [Tooltip("How many particles will we use in the simulation? Controlled by adjusting the WATER LEVEL transform lever in the scene (blue box)")]
        ...
        [ReadOnlyInsp, SerializeField, Tooltip("How many particles, per axis?")]
    ...
    private void UpdateParticleConfig() {
        ...
        maxNumParticles = _numParticlesPerAxis[0] * _numParticlesPerAxis[1] * _numParticlesPerAxis[2];
        numParticles = Mathf.Clamp(numParticles,0,maxNumParticles);
    ...
    [Header("== BOID CONFIGURATIONS ==")]
        ...
        #endif
        [ReadOnlyInsp] public int numBoids = 100;
    */
    ...
    [Header("== GIZMOS CONFIGURATIONS ==")]
        ...
        [Tooltip("What color should the particle velocities be via Gizmos?")]
        public Color gizmos_velocity_color = Color.yellow;
        public Color gizmos_acceleration_color = Color.yellow;
    ...
    int compute_pressure_force_kernel;
    int compute_viscosity_force_kernel;
    ...
    public ComputeBuffer pressure_forces_buffer;
    // Stores the viscosity force of each particle
    public ComputeBuffer viscosity_forces_buffer;
    ...
    void OnDrawGizmos() {
        ...
        float3[] temp_accelerations = new float3[numParticles];
        force_buffer.GetData(temp_accelerations);
        ...
        for(int i = 0; i < numParticles; i++) {
            float3 pos = temp_particles[i].position;
            if (show_particles) {
                Gizmos.color = gizmos_particle_color;
                Gizmos.DrawSphere(new Vector3(pos[0], pos[1], pos[2]), particleRenderRadius);
            }
            if (show_velocities) {
                Gizmos.color = gizmos_velocity_color;
                float3 v = temp_velocities[i];
                Gizmos.DrawRay(pos, new Vector3(v[0], v[1], v[2]));
                //if (v[1] > 0) Debug.Log($"Particle {i} is going upward");
            }
            if (show_forces) {
                Gizmos.color = gizmos_acceleration_color;
                float3 a = temp_accelerations[i];
                Gizmos.DrawRay(pos, new Vector3(a[0], a[1], a[2]));
                //Debug.Log(a);
            }
            /*
    ...
    void InitializeKernels() {
        ...
        compute_pressure_force_kernel = SPHComputeShader.FindKernel("ComputePressureForce");
        compute_viscosity_force_kernel = SPHComputeShader.FindKernel("ComputeViscosityForce");
    ...
    public void UpdateShaderVariables(bool updateDT = false) {
        ...
        SPHComputeShader.SetFloat("radius3", radius2 * smoothingRadius);
        SPHComputeShader.SetFloat("radius4", radius4);
        SPHComputeShader.SetFloat("radius5", radius4 * smoothingRadius);
        ...
        SPHComputeShader.SetFloats("g", g);
        if (updateDT) {
            float t = (dt >= 0) ? dt : Time.deltaTime;
            SPHComputeShader.SetFloat("dt", t);
            //Debug.Log($"Updating dt: {t}");
        }
    }
    ...
    public void InitializeBuffers() {
        ...
        pressure_forces_buffer = new ComputeBuffer(numParticles, sizeof(float)*3);
        viscosity_forces_buffer = new ComputeBuffer(numParticles, sizeof(float)*3);
        ...
        SPHComputeShader.SetBuffer(compute_pressure_force_kernel, "particles", particle_buffer);
        SPHComputeShader.SetBuffer(compute_pressure_force_kernel, "grid", gridBuffer);
        SPHComputeShader.SetBuffer(compute_pressure_force_kernel, "particleNeighbors", particleNeighborsBuffer);
        SPHComputeShader.SetBuffer(compute_pressure_force_kernel, "density", density_buffer);
        SPHComputeShader.SetBuffer(compute_pressure_force_kernel, "pressureForces", pressure_forces_buffer);

        SPHComputeShader.SetBuffer(compute_viscosity_force_kernel, "particles", particle_buffer);
        SPHComputeShader.SetBuffer(compute_viscosity_force_kernel, "grid", gridBuffer);
        SPHComputeShader.SetBuffer(compute_viscosity_force_kernel, "particleNeighbors", particleNeighborsBuffer);
        SPHComputeShader.SetBuffer(compute_viscosity_force_kernel, "density", density_buffer);
        SPHComputeShader.SetBuffer(compute_viscosity_force_kernel, "velocity", velocity_buffer);
        SPHComputeShader.SetBuffer(compute_viscosity_force_kernel, "viscosityForces", viscosity_forces_buffer);
        ...
        SPHComputeShader.SetBuffer(compute_external_acceleration_kernel, "density", density_buffer);
        ...
        SPHComputeShader.SetBuffer(integrate_kernel, "pressureForces", pressure_forces_buffer);
        SPHComputeShader.SetBuffer(integrate_kernel, "viscosityForces", viscosity_forces_buffer);
    ...
    */
    ...
    private void CubeVolumeProcess(int debugNumGridCells, int debugNumParticles) {
        ...
        SPHComputeShader.Dispatch(compute_pressure_force_kernel, Mathf.CeilToInt((float)numParticles / (float)_BLOCK_SIZE),1,1);
        SPHComputeShader.Dispatch(compute_viscosity_force_kernel, Mathf.CeilToInt((float)numParticles / (float)_BLOCK_SIZE),1,1);
        if (verbose_force) {
    ...
    void OnDestroy() {
        ...
        mass_buffer.Dispose();
        temp_buffer.Dispose();
        pressure_forces_buffer.Dispose();
        viscosity_forces_buffer.Dispose();
    }
