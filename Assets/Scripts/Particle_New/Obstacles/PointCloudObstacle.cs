...
public class PointCloudObstacle : MonoBehaviour
{
    ...
    [SerializeField] private bool _RENDER_GIZMOS = true;
    
    void OnDrawGizmos() {
        if (!_RENDER_GIZMOS) return;
        if (pointCloudResolution <= 0f) return;
    ...
    private void GeneratePoints() {
        ...
        for(int t = 0; t < triangles.Length; t += 3) {
            ...
            points.Add(transform.InverseTransformPoint(wv1));
            points.Add(transform.InverseTransformPoint(wv2));
            ...
            for(int x = 0; x < wv1wv2_num; x++) {
                ...
                for(int y = 0; y < wv3_wv1wv2_num; y++) {
                    ...
                    */
                    tempPos = wv1 
                        + (wv1wv2.normalized * (x*pointCloudResolution)) 
                        + (wv3_wv1wv2.normalized * (y*pointCloudResolution));
                    tempPos = plane.ClosestPointOnPlane(tempPos);
